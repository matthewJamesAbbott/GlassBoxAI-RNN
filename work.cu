...
// ============================================================================
// Missing CUDA RNN Facade Components
// To be integrated into the main CUDA RNN implementation
// ============================================================================

// Add to CudaSimpleRNNCell class:
class CudaSimpleRNNCell {
public:
    // ... existing members ...

    // Add optimizer state arrays
    DeviceArray MWih, MWhh, MBh;  // Momentum
    DeviceArray VWih, VWhh, VBh;  // Velocity (for Adam)
    ...
    void SaveWeights(const std::string& filename) {
        ...
        // Download weights from device
        DArray h_Wih, h_Whh, h_Bh;
        Wih.copyToHost(h_Wih);
        Whh.copyToHost(h_Whh);
        Bh.copyToHost(h_Bh);
        ...
        for (int i = 0; i < hiddenSize; i++) {
            for (int j = 0; j < inputSize; j++) {
                out << h_Wih[i * inputSize + j];
                if (j + 1 < inputSize) out << ",";
            }
            out << "\n";
        }
        ...
        for (int i = 0; i < hiddenSize; i++) {
            for (int j = 0; j < hiddenSize; j++) {
                out << h_Whh[i * hiddenSize + j];
                if (j + 1 < hiddenSize) out << ",";
            }
            out << "\n";
        }
    ...
    void LoadWeights(const std::string& filename) {
        ...
        std::string line;
        enum Section { NONE, WIH, WHH, BH } section = NONE;
        std::vector<std::vector<double>> tempWih, tempWhh;
        std::vector<double> tempBh;

        while (std::getline(in, line)) {
            if (line == "#Wih") { section = WIH; continue; }
            else if (line == "#Whh") { section = WHH; continue; }
            ...
            if (section == WIH) tempWih.push_back(vals);
            else if (section == WHH) tempWhh.push_back(vals);
            else if (section == BH) tempBh = vals;
        }
        in.close();

        // Upload to device
        if (!tempWih.empty()) {
            DArray flat(hiddenSize * inputSize);
            for (int i = 0; i < hiddenSize; i++)
                for (int j = 0; j < inputSize; j++)
                    flat[i * inputSize + j] = tempWih[i][j];
            Wih.copyFromHost(flat);
        }

        if (!tempWhh.empty()) {
            DArray flat(hiddenSize * hiddenSize);
            for (int i = 0; i < hiddenSize; i++)
                for (int j = 0; j < hiddenSize; j++)
                    flat[i * hiddenSize + j] = tempWhh[i][j];
            Whh.copyFromHost(flat);
        }

        if (!tempBh.empty()) {
            Bh.copyFromHost(tempBh);
        }
    }
};

// Add to CudaLSTMCell class:
class CudaLSTMCell {
public:
    // ... existing members ...

    // Add optimizer state arrays
    DeviceArray MWf, MWi, MWc, MWo;
    DeviceArray MBf, MBi, MBc, MBo;
    DeviceArray VWf, VWi, VWc, VWo;
    DeviceArray VBf, VBi, VBc, VBo;

    void SaveWeights(const std::string& filename) {
        std::ofstream out(filename);
        if (!out.is_open()) return;

        DArray h_Wf, h_Wi, h_Wc, h_Wo, h_Bf, h_Bi, h_Bc, h_Bo;
        Wf.copyToHost(h_Wf); Wi.copyToHost(h_Wi);
        Wc.copyToHost(h_Wc); Wo.copyToHost(h_Wo);
        Bf.copyToHost(h_Bf); Bi.copyToHost(h_Bi);
        Bc.copyToHost(h_Bc); Bo.copyToHost(h_Bo);

        out << "#Wf\n";
        for (int i = 0; i < hiddenSize; i++) {
            for (int j = 0; j < concatSize; j++) {
                out << h_Wf[i * concatSize + j];
                if (j + 1 < concatSize) out << ",";
            }
            out << "\n";
        }
    ...
    void LoadWeights(const std::string& filename) {
        ...
        std::vector<std::vector<double>> tempWf, tempWi, tempWc, tempWo;
        std::vector<double> tempBf, tempBi, tempBc, tempBo;
        ...
        while (std::getline(in, line)) {
            ...
            switch (section) {
                case WF: tempWf.push_back(vals); break;
                case WI: tempWi.push_back(vals); break;
                case WC: tempWc.push_back(vals); break;
                case WO: tempWo.push_back(vals); break;
                case BF: tempBf = vals; break;
                case BI: tempBi = vals; break;
                case BC: tempBc = vals; break;
                case BO: tempBo = vals; break;
                default: break;
            }
        }
        in.close();

        // Upload to device (flatten 2D to 1D)
        auto uploadMatrix = [this](DeviceArray& dev, const std::vector<std::vector<double>>& mat, int rows, int cols) {
            if (mat.empty()) return;
            DArray flat(rows * cols);
            for (int i = 0; i < rows; i++)
                for (int j = 0; j < cols; j++)
                    flat[i * cols + j] = mat[i][j];
            dev.copyFromHost(flat);
        };

        uploadMatrix(Wf, tempWf, hiddenSize, concatSize);
        uploadMatrix(Wi, tempWi, hiddenSize, concatSize);
        uploadMatrix(Wc, tempWc, hiddenSize, concatSize);
        uploadMatrix(Wo, tempWo, hiddenSize, concatSize);

        if (!tempBf.empty()) Bf.copyFromHost(tempBf);
        if (!tempBi.empty()) Bi.copyFromHost(tempBi);
        if (!tempBc.empty()) Bc.copyFromHost(tempBc);
        if (!tempBo.empty()) Bo.copyFromHost(tempBo);
    }
};

// Add to CudaGRUCell class:
class CudaGRUCell {
public:
    // ... existing members ...

    DeviceArray MWz, MWr, MWh;
    DeviceArray MBz, MBr, MBh;
    DeviceArray VWz, VWr, VWh;
    DeviceArray VBz, VBr, VBh;

    void SaveWeights(const std::string& filename) {
        std::ofstream out(filename);
        if (!out.is_open()) return;

        DArray h_Wz, h_Wr, h_Wh, h_Bz, h_Br, h_Bh;
        Wz.copyToHost(h_Wz); Wr.copyToHost(h_Wr); Wh.copyToHost(h_Wh);
        Bz.copyToHost(h_Bz); Br.copyToHost(h_Br); Bh.copyToHost(h_Bh);

        out << "#Wz\n";
        for (int i = 0; i < hiddenSize; i++) {
            for (int j = 0; j < concatSize; j++) {
                out << h_Wz[i * concatSize + j];
                if (j + 1 < concatSize) out << ",";
            }
            out << "\n";
        }
    ...
    void LoadWeights(const std::string& filename) {
        // Similar implementation to LSTM
        // ... (follow same pattern as LSTM LoadWeights)
    }
};

// Add to CudaOutputLayer:
class CudaOutputLayer {
public:
    // ... existing members ...

    DeviceArray MW, MB;  // Momentum
    DeviceArray VW, VB;  // Velocity
    ...
    void LoadWeights(const std::string& filename) {
        // Similar pattern as other layers
    }
};

// Add to CudaRNNFacade class:
class CudaRNNFacade {
private:
    // ... existing members ...

    // Add gradient history tracking
    vector<DArray> gradientHistory;

public:
    // Add complete GRU backward implementation
    void gruBackward(int layer, DeviceArray& dH, DeviceArray& Z, DeviceArray& R,
                     DeviceArray& HTilde, DeviceArray& PrevH, DeviceArray& Input,
                     double clipVal, DeviceArray& dInput, DeviceArray& dPrevH) {
        int hidSize = hiddenSizes[layer];
        int inSize = (layer == 0) ? inputSize : hiddenSizes[layer-1];
        int concatSize = inSize + hidSize;

        // Allocate temporary gradient arrays
        DeviceArray dZ(hidSize), dR(hidSize), dHTilde(hidSize);
        DeviceArray Concat(concatSize), ConcatR(concatSize);

        // Concat [Input, PrevH]
        int blocks = (concatSize + BLOCK_SIZE - 1) / BLOCK_SIZE;
        k_concat<<<blocks, BLOCK_SIZE>>>(Concat.d_ptr, Input.d_ptr, PrevH.d_ptr, inSize, hidSize);

        // Concat [Input, R * PrevH] for hidden candidate
        k_concat<<<blocks, BLOCK_SIZE>>>(ConcatR.d_ptr, Input.d_ptr, PrevH.d_ptr, inSize, hidSize);
        // Apply reset gate (element-wise multiply on hidden part)
        // This would need a custom kernel

        // Compute dZ, dR, dHTilde using custom kernels
    ...
    void saveModel(const std::string& filename) {
        ...
        out.write((char*)&outputSize, sizeof(int));
        int numLayers = hiddenSizes.size();
        out.write((char*)&numLayers, sizeof(int));
        out.write((char*)hiddenSizes.data(), numLayers * sizeof(int));
        int ct = (int)cellType;
        out.write((char*)&ct, sizeof(int));

        // Save each layer's weights to separate files
        for (size_t i = 0; i < hiddenSizes.size(); i++) {
            std::string layerFile = filename + ".layer" + std::to_string(i);
            switch (cellType) {
                case ctSimpleRNN:
                    simpleCells[i]->SaveWeights(layerFile);
                    break;
                case ctLSTM:
                    lstmCells[i]->SaveWeights(layerFile);
                    break;
                case ctGRU:
                    gruCells[i]->SaveWeights(layerFile);
                    break;
            }
        }

        // Save output layer
        std::string outFile = filename + ".output";
        outputLayer->SaveWeights(outFile);
    ...
    void loadModel(const std::string& filename) {
        ...
        in.read((char*)&outputSize, sizeof(int));
        in.read((char*)&numLayers, sizeof(int));
        hiddenSizes.resize(numLayers);
        in.read((char*)hiddenSizes.data(), numLayers * sizeof(int));
        in.read((char*)&ct, sizeof(int));
        ...
        for (size_t i = 0; i < hiddenSizes.size(); i++) {
            ...
            switch (cellType) {
                case ctSimpleRNN:
                    if (simpleCells[i])
                        simpleCells[i]->LoadWeights(layerFile);
                    break;
                case ctLSTM:
                    if (lstmCells[i])
                        lstmCells[i]->LoadWeights(layerFile);
                    break;
                case ctGRU:
                    if (gruCells[i])
                        gruCells[i]->LoadWeights(layerFile);
                    break;
            }
        }
        ...
        // Load output layer
        std::string outFile = filename + ".output";
        outputLayer->LoadWeights(outFile);

        std::cout << "Model loaded from " << filename << std::endl;
    }

    // Add histogram computation
    struct Histogram {
    ...
    Histogram getHiddenStateHistogram(int layerIdx, int timestep, int numBins = 10) {
        Histogram hist;
        if (timestep < 0 || timestep >= sequenceLen) return hist;
        if (!caches[timestep].H) return hist;

        DArray h;
        caches[timestep].H->copyToHost(h);
        if (h.empty()) return hist;

        double minVal = *std::min_element(h.begin(), h.end());
        double maxVal = *std::max_element(h.begin(), h.end());
        if (maxVal == minVal) maxVal = minVal + 1;

        double binWidth = (maxVal - minVal) / numBins;

        hist.rangeMin.resize(numBins);
        hist.rangeMax.resize(numBins);
        hist.count.resize(numBins, 0);
        hist.percentage.resize(numBins);

        for (int i = 0; i < numBins; i++) {
            hist.rangeMin[i] = minVal + i * binWidth;
            hist.rangeMax[i] = minVal + (i + 1) * binWidth;
        }
        ...
        for (int i = 0; i < numBins; i++) {
            hist.percentage[i] = (double)hist.count[i] / h.size() * 100.0;
        }

        return hist;
    }

    // Add gate saturation analysis
    struct GateSaturation {
        int nearZeroCount;
        int nearOneCount;
        int totalCount;
        double nearZeroPct;
    ...
    GateSaturation getGateSaturation(GateType gateType, int layerIdx, int timestep, double threshold = 0.05) {
        GateSaturation stats = {0, 0, 0, 0.0, 0.0};

        if (timestep < 0 || timestep >= sequenceLen) return stats;
        ...
        if (gate.empty()) return stats;

        stats.totalCount = gate.size();
        for (double val : gate) {
            if (val < threshold) stats.nearZeroCount++;
            if (val > (1.0 - threshold)) stats.nearOneCount++;
        }

        stats.nearZeroPct = (double)stats.nearZeroCount / stats.totalCount * 100.0;
        stats.nearOnePct = (double)stats.nearOneCount / stats.totalCount * 100.0;
    ...
    LayerNormStats getLayerNormStats(int layerIdx, int timestep) {
        LayerNormStats stats = {0.0, 1.0, 1.0, 0.0};

        if (timestep < 0 || timestep >= sequenceLen) return stats;
        if (!caches[timestep].H) return stats;

        DArray h;
        caches[timestep].H->copyToHost(h);
        if (h.empty()) return stats;
        ...
        stats.mean = sum / h.size();
        stats.variance = sumSq / h.size() - stats.mean * stats.mean;

        return stats;
    }

    // Add attention mechanism support (placeholder for future)
    DArray getAttentionWeights(int timestep) {
        // Placeholder - would need attention mechanism implementation
        return DArray();
    }

    DArray getAttentionContext(int timestep) {
        // Placeholder - would need attention mechanism implementation
        return DArray();
    }

    // Add state injection methods
    void injectHiddenState(int layerIdx, const DArray& values) {
        if (layerIdx < 0 || layerIdx >= (int)hiddenSizes.size()) return;
        if (layerIdx >= (int)d_StateH.size()) return;

        int copySize = std::min((int)values.size(), hiddenSizes[layerIdx]);
        d_StateH[layerIdx].copyFromHost(values.data(), copySize);
    }

    void injectCellState(int layerIdx, const DArray& values) {
        if (cellType != ctLSTM) return;
        if (layerIdx < 0 || layerIdx >= (int)hiddenSizes.size()) return;
        if (layerIdx >= (int)d_StateC.size()) return;

        int copySize = std::min((int)values.size(), hiddenSizes[layerIdx]);
        d_StateC[layerIdx].copyFromHost(values.data(), copySize);
    }

    // Add state reset with specific values
    void resetHiddenState(int layerIdx, double value = 0.0) {
        if (layerIdx < 0 || layerIdx >= (int)hiddenSizes.size()) return;
        if (layerIdx >= (int)d_StateH.size()) return;

        d_StateH[layerIdx].fill(value);
    }
    ...
    void resetCellState(int layerIdx, double value = 0.0) {
        ...
        d_StateC[layerIdx].fill(value);
    }

    void resetAllStates(double value = 0.0) {
        for (size_t i = 0; i < d_StateH.size(); i++) {
            d_StateH[i].fill(value);
            if (cellType == ctLSTM && i < d_StateC.size()) {
                d_StateC[i].fill(value);
            }
        }
    }
};
...
int main(int argc, char* argv[]) {
    ...
    int bpttSteps = 0;
    int epochs = 100;
    string inputFile, targetFile, outputFile, modelFile;
    ...
    for (int i = 2; i < argc; i++) {
        ...
        else if (arg == "--model-file" && i+1 < argc) modelFile = argv[++i];
    ...
    if (command == "save-model") {
        ...
        // Optionally train before saving
        if (!inputFile.empty() && !targetFile.empty()) {
            DArray2D inputs = LoadCSV(inputFile, inputSize);
            DArray2D targets = LoadCSV(targetFile, outputSize);

            if (!inputs.empty() && !targets.empty()) {
                cout << "Training before save...\n";
    ...
    if (command == "load-model") {
        if (modelFile.empty()) {
            cerr << "Error: --model-file required for load-model\n";
            return 1;
        }
        ...
        // Create a temporary RNN to load into
        // We'll read the metadata first, then recreate with correct dimensions
        CudaRNNFacade rnn(1, {1}, 1, cellType, activation,
                          outputActivation, lossType, learningRate, gradientClip, bpttSteps);
        ...
        // Optionally run prediction with loaded model
        if (!inputFile.empty()) {
            DArray2D inputs = LoadCSV(inputFile, rnn.getInputSize());
            if (!inputs.empty()) {
                cout << "Running prediction with loaded model...\n";
